...
const OrderHistory = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Check if data is null or data.data is null, set orders to an empty array
      if (!data || !data.data) {
        setOrders([]);
      } else {
        setOrders(data.data);
  ...
  const filteredOrders = orders ? orders.filter((order) => order.status === "finished" || order.status === "cancelled") : [];
